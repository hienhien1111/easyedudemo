...
echo ""
echo "     Lưu ý NEXT_PUBLIC_API_URL phải trỏ qua nginx (port 80):"
echo "     NEXT_PUBLIC_API_URL=http://localhost/api"
echo "     (nếu truy cập từ xa: thay localhost bằng IP server)"
echo ""
...
echo ""
echo "  6. Deploy lần đầu để khởi động containers (bao gồm nginx):"
echo "     cd $PROJECT_DIR"
echo "     docker compose -f docker-compose.prod.yml up -d --build"
echo ""
echo "  7. Kiểm tra nginx hoạt động:"
echo "     curl http://localhost/nginx-health     # kỳ vọng: ok"
echo "     curl http://localhost/api              # kỳ vọng: NestJS response"
echo "     curl http://localhost                  # kỳ vọng: Next.js page"
echo ""
echo "  8. Push code lên main và kiểm tra GitHub Actions!"
echo "     (CI/CD sẽ tự động deploy, nginx reload khi có thay đổi)"
echo ""
echo "  Webhook status: sudo systemctl status eedemo-webhook"
echo "  Deploy logs:    tail -f /var/log/eedemo-deploy.log"
echo "  Nginx logs:     docker compose -f $PROJECT_DIR/docker-compose.prod.yml logs nginx"
echo "═══════════════════════════════════════════════"
